 ...
 */
async function resolveCurrentTabRef(ctx: ExecutionContext): Promise<TabRef | undefined> {
  const hostId = ctx.getHostTabId?.();
  if (hostId !== undefined) {
    const t = await ctx.chrome.tabs.get(hostId);
    const id = t?.id ?? hostId;
    const winId = t?.windowId ?? (await getActiveWindowId(ctx));
    const all = await getWindowTabs(ctx.chrome, winId);
    const index = all.findIndex((tab) => tab.id === id) + 1;
    return { id, index: index || 1, title: t?.title ?? '(page)' };
  }
...
export async function resolveTabRef(
  ...
  if (!arg || arg === 'current') {
    return resolveCurrentTabRef(ctx);
  }
